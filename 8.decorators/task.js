...
function cachingDecoratorNew(func) {
  ...
  return function (...args) {
    let hash = md5(args);
    let cashLength = Object.keys(cash).length;
    let res = func(...args);
    ...
    if (!(hash in cash)) {
      if (cashLength < 5) {
        ...
        let arrCash = Object.entries(cash);
        arrCash.shift();
        cash = Object.fromEntries(arrCash);
        cash[hash] = res;
      }
      return 'Вычисляем: ' + res;
    } else {
      return 'Из кеша: ' + cash[hash];
    }
  };
}
...
setTimeout(() => {
  console.log(upgradedSendSignal.count, 'count'); // было выполнено 3 отправки сигнала
  console.log(upgradedSendSignal.allCount, 'allCount'); // было выполнено 6 вызовов декорированной функции
}, 7000);
